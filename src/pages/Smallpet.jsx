...
import { getSmallpetProducts, addToCart, getMediaUrl } from '../api/api'
...
function Smallpet() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await getSmallpetProducts()
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="text-sm">
            ...
            <span className="text-gray-900 font-medium">SmallPets</span>
          </nav>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 underline">SmallPets</h1>
          ...
          <div className={`lg:col-span-9 ${!filtersOpen ? 'block' : 'hidden lg:grid'} grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6`}>
            {filtered.map((product, index) => (
              <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                <div className="relative">
                  <img 
                    onClick={() => navigate(`/smallpet/${product.id}`)}
