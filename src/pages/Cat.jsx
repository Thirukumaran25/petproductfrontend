...
import { getCatProducts, addToCart, getMediaUrl } from '../api/api'
...
function Cat() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await getCatProducts()
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 underline">Cat</h1>
          ...
          <div className={`lg:col-span-9 ${!filtersOpen ? 'block' : 'hidden lg:grid'} grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6`}>
            {filtered.map((product, index) => (
              <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                <div className="relative">
                  <img 
                    onClick={() => navigate(`/cat/${product.id}`)}
